...
│   ├── render                     → Visual QA: DOCX → PDF/PNG via LibreOffice (skips with exit 0 when no renderer)
...
└── assets/
    ├── reference.docx             → CJK-friendly pandoc reference (SimSun/Times body with first-line indent, pure-black headings, code shading, table borders); the md2docx pipeline applies it automatically
    └── templates/
...
- **Editing**: The engine validates internally; after saving, spot-check high-risk areas; for layout-sensitive edits, run `./scripts/render <file.docx>` to visually confirm the result
- **md2docx**: Use the converter's own feedback, verify that the resulting package opens cleanly, and render it for visual review with `./scripts/render <file.docx>`. `./scripts/docx validate` belongs to the C# Create route and is not the md2docx validator.
- **Visual QA skip rule**: When no LibreOffice renderer is available, `./scripts/render` prints a notice and exits 0 — visual QA is skipped, not failed; only a failed PDF conversion exits non-zero (PNG page rendering is best-effort and only warns). Whenever visual QA was skipped, the delivery must state so explicitly.
